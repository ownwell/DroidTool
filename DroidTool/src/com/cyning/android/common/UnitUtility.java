package com.cyning.android.common;

/**
 * @author : 桥下一粒砂 chenyoca@gmail.com date : 2013-4-7 计量单位工具类
 */
public class UnitUtility {
	 ...
	 * 
	 * @param baseUnit
	 *            基本单位。如果一个数没有达到最低计算数值，则用基本返回。例如，计算距离为千米（1000）,如果给定数据为123，则返回
	 *            "123 米"。
	 * @param formatUnit
	 *            计量单位。只计算一个量级时，设置此单位。例如，计算距离为千米，如果给定数据为 123456789，则返回
	 *            123456.79千米。
	 * @return 计量单位工具类对象本身
	 */
	...
	/**
	 * 分段计算。分段计算几个量级时，设置此参数，最基本单位开始。 e.g:
	 * unitUtility.sections("B","KB","MB","GB","TB")
	 * 
	 * @param sections
	 *            分段单位数组
	 * @return 计量单位工具类对象本身
		...
		if (sections == null) {
			if (baseUnit == null || unit == null) {
				throw new IllegalArgumentException(
						"Method 'base(String baseUnit)' and 'unit(String unit)' not makeCall!");
			}
